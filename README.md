...
- zkEVM-compatible infrastructure
- custom low-level virtual machine behavior

Although Solidity contracts look very similar to Ethereum contracts, zkSync uses:

- different low-level opcodes
- different compilation targets
- different VM execution behavior

Because of this, standard Foundry output is not fully compatible with zkSync.
...
- zkSync-specific compilation
- zkSync-compatible bytecode
- zkSync VM support

As a result:

- normal `forge build` output is insufficient
- zkSync requires a modified Foundry toolkit
...
is a fork of Foundry designed specifically for:

- zkSync development
- zkSync compilation
- zkSync deployment workflows

It extends:

- `forge`
- `cast`
- other Foundry tooling

with zkSync-specific functionality.
...
- your environment becomes zkSync-enabled
- some standard workflows may behave differently
...
2. compile using `--zksync`
3. deploy/interact with zkSync
4. restore vanilla Foundry afterward
...
```bash id="f3n8ha"
curl -L https://raw.githubusercontent.com/matter-labs/foundry-zksync/main/foundryup-zksync/install | bash
foundryup-zksync
source ~/.zshrc
source ~/.zshenv
```
...
- do NOT clone inside your main Foundry project
- keep separate toolchain directories
...
A different version output indicates:

- zkSync Foundry installed successfully
...
This switches the active binaries to:

- zkSync-enabled `forge`
- zkSync-enabled `cast`
...
| `foundryup`        | Switch back to standard (Vanilla) Foundry |
...
- changes compilation behavior
- generates zkSync-compatible artifacts
- targets the zkSync VM
...
the generated low-level bytecode differs between:

- Ethereum
- zkSync

This happens because zkSync:

- supports custom opcodes
- optimizes for zero-knowledge execution
- uses a different VM architecture internally
...
- EVM-compatible artifacts

zkSync Foundry generates:

- zkSync-compatible artifacts

Therefore:

- artifacts may not be interchangeable
- deployments require the correct toolchain
...
- Ethereum Foundry workflows
- zkSync Foundry workflows
...
- `foundry-zksync` is a specialized Foundry fork
- Installing zkSync Foundry overrides standard binaries
- Use `forge build --zksync` for zkSync compilation
- `foundryup-zksync` and `foundryup` allow switching environments
- zkSync compilation generates different output artifacts
...
- [zkSync Developer Docs](https://docs.zksync.io?utm_source=chatgpt.com)
- [Ethereum Layer 2 Overview](https://ethereum.org/en/layer-2/?utm_source=chatgpt.com)
- [Course ZKsync compilation guide](https://github.com/Cyfrin/foundry-full-course-cu?tab=readme-ov-file#compiling-to-zksync-in-foundry-zksync)
...
- a modified virtual machine
- zkSync Era VM
- custom low-level execution behavior

Because of this, standard EVM artifacts are not fully compatible with zkSync.

To compile specifically for zkSync:
...
- zkSync-compatible bytecode
- zkSync compilation artifacts
- Era VM deployment outputs

Unlike `/out`, these artifacts are specifically generated for:

- zkSync Era
- zkSync deployment tooling
- zkSync-compatible execution
...
| -------- | ------------- | ---------------------- |
| `/out`   | Ethereum EVM  | `forge build`          |
| `/zkout` | zkSync Era VM | `forge build --zksync` |
...
the generated bytecode differs because:

- zkSync supports custom opcodes
- zkSync optimizes for zero-knowledge proofs
- Era VM differs internally from the EVM
...
This remains the default workflow for most Ethereum development.
...
- zkSync uses a different VM architecture than Ethereum
- zkSync compilation targets the Era VM
- `foundryup-zksync` enables zkSync workflows
- `foundryup` restores vanilla Ethereum Foundry
...
- [Foundry zkSync GitHub Repository](https://github.com/matter-labs/foundry-zksync?utm_source=chatgpt.com)
- [zkSync Official Website](https://zksync.io?utm_source=chatgpt.com)
- [zkSync Era Documentation](https://docs.zksync.io/zksync-era?utm_source=chatgpt.com)
- [Ethereum Layer 2 Overview](https://ethereum.org/en/layer-2/?utm_source=chatgpt.com)
...
> Deploying to a local ZKsync node ensures your contracts work correctly in the ZKsync Era VM, not just in a standard EVM environment.

This tool acts similarly to:

- standard Anvil
- local Ethereum development nodes

but specifically targets:

- zkSync Era
- zkSync Layer 2 testing
- local zkSync-compatible deployments
...
- zkSync uses a different VM internally
- compilation differs from Ethereum
- execution behavior may vary

Running contracts locally on zkSync helps ensure:

- compatibility
- deployment correctness
- zkSync-specific behavior works properly
...
The newer workflow simplifies everything significantly.
...
- local zkSync nodes can run directly
- setup is much easier
- deployment workflows are streamlined
...
Older installations may not include:

- `anvil-zksync`
- latest zkSync tooling
- updated binaries
...
This launches a local zkSync development blockchain.
...
⚠️ Port may vary depending on configuration.

This RPC URL is used for:
...
This helps simulate realistic zkSync conditions.
...
| ------------- | ------------- | ---------------------- |
| Target        | Ethereum EVM  | zkSync Era             |
| VM            | EVM           | zkSync VM              |
| Compilation   | `forge build` | `forge build --zksync` |
| Output Folder | `/out`        | `/zkout`               |
| Layer         | L1            | Layer 2                |
...
- zkSync execution behavior
- zkSync VM assumptions
- Layer 2-specific workflows
...
- Updated `foundry-zksync` is required
- `anvil-zksync` provides funded test accounts and RPC endpoints
- Contracts should be tested directly on zkSync environments
- zkSync development uses `forge build --zksync`
- `anvil-zksync` improves local Layer 2 development workflows
...
- simulate zkSync behavior
- test Layer 2 workflows
- interact with a zkSync-compatible chain

⚠️ This setup is optional.

If configuring the tooling becomes difficult, you can continue using:

- standard Anvil
- local Ethereum testing workflows
...
- test contracts in a zkSync environment
- validate zkSync compatibility
- simulate Layer 2 execution
- debug deployments locally

Since zkSync differs internally from Ethereum:

- local zkSync testing is valuable
- EVM-only testing may not catch all issues
...
| Tool       | Purpose                    |
| ---------- | -------------------------- |
| Docker     | Runs the local zkSync node |
| Node.js    | JavaScript runtime         |
| npm        | Package manager            |
| zksync-cli | zkSync environment manager |
...
Docker icon → Quit Docker Desktop
Option 2 — Terminal
...
- configures the local zkSync environment
- initializes development settings
...
- starts a zkSync node
- launches Docker containers
- runs the environment in the background
...
This spins up a ZKsync node inside Docker running in the background. Verify it's running:

> **Note:** Docker must be running before executing `npx zksync-cli dev start`, otherwise the command will fail.
...
- zkSync requires additional flags
- deployment tooling differs slightly
...
Use `forge create` with the `--zksync` and `--legacy` flags:

```bash
forge create <ContractName> \
  --rpc-url <YOUR_ZKSYNC_RPC_URL> \
  --private-key $PRIVATE_KEY \
  --zksync \
  --legacy
```

> **Note:** `forge script` is not well supported on ZKsync — use `forge create` instead.
...
- zkSync-compatible compilation
- zkSync deployment behavior
- Era VM targeting
...
- compatibility with zkSync deployment flows
- transaction formatting support
...
- `forge script` support on zkSync is limited
- `forge create` is more reliable

Therefore:

- deployments are recommended via `forge create`
...
⚠️ RPC URL may vary depending on configuration.
...
| ------------- | -------------- | -------------------------------- |
| Local Node    | Anvil          | zkSync CLI + Docker              |
| VM            | EVM            | zkSync Era VM                    |
| Compilation   | `forge build`  | `forge build --zksync`           |
| Deployment    | `forge create` | `forge create --zksync --legacy` |
| Output Folder | `/out`         | `/zkout`                         |
...
- `zksync-cli` manages the local environment
- `forge build --zksync` compiles zkSync-compatible contracts
- `forge create --zksync --legacy` is the recommended deployment flow
- `forge script` support on zkSync is currently limited
- Local zkSync testing improves Layer 2 compatibility validation
...
These transaction types evolved over time through:
...
Understanding transaction types is important because:

- different chains may use different defaults
- deployment tools may generate different transaction formats
- zkSync introduces custom transaction behavior
...
When deploying contracts with Foundry:
...
⚠️ This is currently the standard default on Ethereum-compatible chains.
...
ZKsync requires `--legacy` when using `forge create` to enforce type `0x0` compatibility.

> **Note:** `forge script` works in some ZKsync scenarios but may fail unpredictably — for this course, assume scripting does not work with ZKsync and use `forge create` instead.
...
- some zkSync tooling expects legacy formatting
- compatibility can vary
- zkSync transaction handling differs from Ethereum
...
Over time, new transaction types were introduced through EIPs.
...
| `0x71` | zkSync-specific transaction |

| Type | Hex    | Name                                                                                            | Description                                                                                                              |
| ---- | ------ | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| 0    | `0x0`  | Legacy                                                                                          | Original Ethereum transaction type                                                                                       |
| 1    | `0x1`  | EIP-2930                                                                                        | Adds an access list of addresses and storage keys                                                                        |
| 2    | `0x2`  | [EIP-1559](https://eips.ethereum.org/EIPS/eip-1559)                                             | Fee market change — **current EVM default**                                                                              |
| 113  | `0x71` | [EIP-712 / ZKsync](https://docs.zksync.io/zk-stack/concepts/transaction-lifecycle#eip-712-0x71) | ZKsync-native type, enables [account abstraction](https://docs.zksync.io/build/developer-reference/account-abstraction/) |
...
Modern Ethereum transactions typically use:
...
This transaction type supports advanced zkSync functionality such as:

- account abstraction
- zkSync-native features
...
zkSync integrates this more deeply than standard Ethereum.
...
```text id="r6t2kp"
/broadcast/<chain-id>/run-latest.json
```
...
| ------------------- | ------------ | -------------- |
| Default Type        | `0x2`        | Often `0x0`    |
| Legacy Support      | Yes          | Yes            |
| EIP-1559            | Yes          | Partial/varies |
| Custom Types        | No           | Yes (`0x71`)   |
| Account Abstraction | Limited      | Native support |
...
is commonly recommended for compatibility.
...
- prefer `forge create`
- especially during zkSync development
...
- `--legacy` forces type `0x0`
- zkSync introduces custom transaction type `0x71`
- zkSync transaction handling differs from standard Ethereum
- `/broadcast` stores deployment transaction data
- `run-latest.json` contains transaction metadata
- `forge create --zksync --legacy` is commonly recommended for zkSync
...
- [EIP-2930 Specification](https://eips.ethereum.org/EIPS/eip-2930?utm_source=chatgpt.com)
- [Cyfrin Blog — EIP-4844](https://www.cyfrin.io/blog/what-is-eip-4844?utm_source=chatgpt.com)
- [Foundry Book](https://book.getfoundry.sh/?utm_source=chatgpt.com)
- [zkSync Official Website](https://zksync.io?utm_source=chatgpt.com)
...
- explored ZkSync
- learned local and testnet deployment workflows
...
- Layer 1 behavior
- Ethereum deployment mechanics
- real blockchain execution

However, most modern projects prefer deploying to:

- Layer 2 (L2) networks

instead of directly to Ethereum Layer 1 (L1).
...
More complex contracts require:
...
Deploying even a minimal contract like `SimpleStorage` on Ethereum mainnet is expensive. To estimate the cost, check the `gasUsed` value in your `/broadcast/run-latest.json` and convert it from hex:
...
into a human-readable decimal number.
...
```text id="w2m5pk"
0.000279288255846978 ETH
```

Approximate cost:

- around `$7 USD` at the time of the lesson
...
- several dollars
- tens of dollars during congestion

Large smart contracts with:
...
| zkSync Era       | Much lower          |
| Other L2s        | Lower than Ethereum |
...
2. choose the zkSync Sepolia network
3. copy the RPC URL
...
1. deploy locally
2. inspect gas usage
3. estimate Ethereum mainnet costs
4. compare against Layer 2 costs
...
- [L2Fees.info](https://l2fees.info/?utm_source=chatgpt.com)

This website compares:
...
Most modern dApps now:

- launch directly on L2
- support multiple chains
- optimize for low-cost transactions
...
- Foundry stores deployment gas data in `/broadcast`
- `cast --to-base` converts hexadecimal gas values
- Layer 2 solutions dramatically reduce costs
- zkSync provides Ethereum-compatible low-cost deployments
- Most modern projects prefer deploying to L2s
- zkSync Sepolia deployment is similar to local zkSync deployment
...
- [Foundry Book](https://book.getfoundry.sh/?utm_source=chatgpt.com)
- [Ethereum Layer 2 Overview](https://ethereum.org/en/layer-2/?utm_source=chatgpt.com)
- [Etherscan Gas Tracker](https://etherscan.io/gastracker?utm_source=chatgpt.com)
